...
2. **Orquestador Maestro y Renderizado Condicional:** El archivo original (ej. `[Nombre].tsx`) actúa EXCLUSIVAMENTE como un orquestador lógico. Queda **ESTRICTAMENTE PROHIBIDO** montar ambas vistas a la vez y ocultarlas con CSS (`hidden lg:block` vs `block lg:hidden`), ya que esto duplica árboles del Virtual DOM y ciclos de vida de React, penalizando severamente la memoria y CPU. El orquestador DEBE renderizar de forma condicional utilizando el hook centralizado `useIsMobile(1024)`: `{isMobile ? <MobileView logic={logic} /> : <DesktopView logic={logic} />}`.
3. **SSoT (Single Source of Truth) Lógico:** Toda la lógica de negocio y manejo de estados DEBE residir en un único Hook centralizado (ej. `use[Nombre]Logic.ts`), el cual se inyecta en la vista activa para no perder el estado al cambiar de resolución.
