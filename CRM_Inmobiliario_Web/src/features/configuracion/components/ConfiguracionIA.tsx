    ...
    <div className="space-y-6">
      ...
      <section className="space-y-6 bg-slate-100/50 p-8 rounded-[40px] border border-slate-200/60 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-100">
        ...
        <div className="max-w-xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              onClick={() => handleVectorizeDocs(false)}
              disabled={isVectorizingDocs}
            ...
            >
              {isVectorizingDocs ? <Loader2 size={18} className="animate-spin" /> : <Database size={18} />}
              Vectorizar Faltantes
            </button>

            <button
              onClick={() => setShowForceDocsModal(true)}
              disabled={isVectorizingDocs}
              className="cursor-pointer flex-1 px-6 py-3 bg-white text-rose-600 border-2 border-rose-100 rounded-xl font-bold flex justify-center items-center gap-2 hover:bg-rose-50 hover:border-rose-200 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <AlertTriangle size={18} />
          ...
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6">
              <div className="flex justify-between items-start mb-6">
                ...
                <button onClick={() => setShowForceModal(false)} className="cursor-pointer text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100 transition-colors">
                  <X size={20} />
                </button>
              </div>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6">
              <div className="flex justify-between items-start mb-6">
                ...
                <button onClick={() => setShowForceDocsModal(false)} className="cursor-pointer text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100 transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setShowForceDocsModal(false)}
                  className="cursor-pointer flex-1 px-4 py-3 font-bold text-slate-700 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors"
                >
                  Cancelar
                </button>
                <button
                  onClick={() => handleVectorizeDocs(true)}
                  className="cursor-pointer flex-1 px-4 py-3 font-bold text-white bg-rose-600 rounded-xl hover:bg-rose-700 transition-colors shadow-sm shadow-rose-600/20"
                >
                  Sí, Forzar
                </button>
