...
import { UserX, Search, ShieldAlert, Loader2 } from 'lucide-react';
...
export const ReasignacionAgenteModal: React.FC<ReasignacionAgenteModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-[32px] max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        <div className="p-8 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-slate-800 tracking-tight">Desactivar Agente</h3>
              <p className="text-slate-500 font-medium">Reasignación de Cartera de {agenteDesactivarNombre}</p>
            </div>
          </div>
        ...
        <div className="p-8 flex-1 overflow-y-auto">
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-800 placeholder:text-slate-400"
            />
          </div>
              ...
              resultados.map(agente => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-500">{agente.email}</p>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3 shrink-0">
          <button
            ...
            className="flex-1 px-4 py-4 font-bold text-slate-700 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-colors disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            className="flex-1 px-4 py-4 font-bold text-white bg-rose-600 rounded-2xl hover:bg-rose-700 transition-colors shadow-sm shadow-rose-600/20 disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isDesactivando ? (
              <>
