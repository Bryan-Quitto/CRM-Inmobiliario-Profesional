...
interface EliminarAgenteModalProps {
  ...
  agenteEliminarNombre: string;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const EliminarAgenteModal: React.FC<EliminarAgenteModalProps> = ({
  agenteEliminarId,
  agenteEliminarNombre,
  isOpen,
  onClose,
  onSuccess,
}) => {
  const { agentes, isLoading: isAgentesLoading } = useAgentes();
  const { mutateAsync: eliminar, isLoading: isEliminando } = useEliminarAgente();
  ...
  const agentesValidos = useMemo(() => {
    return agentes?.filter(a => a.id !== agenteEliminarId && a.activo) || [];
  }, [agentes, agenteEliminarId]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-[32px] max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[85vh]">
        <div className="p-4 sm:p-6 border-b border-rose-100 shrink-0 bg-rose-50/30">
          <div className="flex items-center gap-4 mb-4 w-full">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-2xl font-black text-rose-700 tracking-tight break-words">Eliminar Cuenta</h3>
              <p className="text-rose-600/80 font-medium break-words">Eliminación permanente de {agenteEliminarNombre}</p>
            </div>
          </div>
          <div className="space-y-2">
            <p className="text-sm font-medium text-slate-700 break-words">
              Estás a punto de eliminar esta cuenta. Se revocará el acceso inmediatamente y a los <strong>30 días</strong> los datos personales serán anonimizados por completo por obligación legal.
            </p>
            <p className="text-sm font-medium text-slate-700 break-words">
              ¿A qué agente deseas reasignar TODAS las Propiedades y Contactos de este usuario para no perder el seguimiento?
            </p>
          </div>
        </div>
