...
export const DynamicFilterInput = ({ contactos, filterDef, filters, onChange, onRemove, isRemovable }: DynamicFilterInputProps) => {
  ...
  const uniqueValues = useMemo(() => {
    if (type !== 'text' || !contactos) return [];
    const values = new Set<string>();
    contactos.forEach(p => {
      const val = p[key as keyof Contacto];
      ...
      case 'date':
        return (
          <div className="flex items-center gap-3">
            <input 
              type="date" 
              placeholder={minLabel || 'Desde'}
              value={(filters[`${key}Min`] as string) || ''}
              onChange={e => onChange(`${key}Min`, e.target.value)}
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-all"
            />
            <span className="text-slate-400 font-bold">-</span>
            <input 
              type="date" 
              placeholder={maxLabel || 'Hasta'}
              value={(filters[`${key}Max`] as string) || ''}
              onChange={e => onChange(`${key}Max`, e.target.value)}
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-all"
            />
          </div>
