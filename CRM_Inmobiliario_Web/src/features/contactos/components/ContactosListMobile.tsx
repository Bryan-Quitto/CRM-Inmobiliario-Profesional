...
export const ContactosListMobile: React.FC<ContactosListMobileProps> = ({ logic }) => {
  return (
    <div className="w-full bg-slate-50 min-h-screen relative font-sans antialiased space-y-4 p-4 pb-6 overflow-x-hidden min-w-0">
      ...
      {logic.totalPages > 1 && (
        <div className="w-full mt-6 mb-2 flex justify-center items-center gap-2 flex-wrap min-w-0">
          <button 
            ...
            className={`px-3 py-2 rounded-xl text-sm font-bold transition-all ${logic.currentPage === 1 ? 'bg-slate-100 text-slate-400' : 'bg-white text-slate-700 border border-slate-200'}`}
          >
            Ant
          </button>
          ...
          <div className="flex flex-wrap items-center justify-center gap-1 min-w-0">
            {logic.visiblePages.map(page => (
              <button
                ...
                className={`w-9 h-9 rounded-xl text-xs font-bold transition-all flex items-center justify-center ${logic.currentPage === page ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200'}`}
              >
                {page}
              </button>
            ))}
          ...
          <button 
            ...
            className={`px-3 py-2 rounded-xl text-sm font-bold transition-all ${logic.currentPage === logic.totalPages ? 'bg-slate-100 text-slate-400' : 'bg-white text-slate-700 border border-slate-200'}`}
          >
            Sig
          </button>
        </div>
