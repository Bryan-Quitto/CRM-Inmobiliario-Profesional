...
export const ClienteCard = ({
  ...
  const currentEtapa = isOwnerMode ? cliente.estadoPropietario : cliente.etapaEmbudo;
            ...
            <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-100 rounded-2xl shadow-2xl z-[50] py-2 animate-in fade-in zoom-in duration-200 origin-top-right backdrop-blur-xl bg-white/95">
              {currentOptions.map((etapa) => (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onStageChange(cliente.id, etapa.value, undefined, isOwnerMode ? 'propietario' : 'prospecto');
                  }}
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2">
          ...
          {cliente.esPropietario && (
            <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider border ${
              cliente.propiedadesCaptadas && cliente.propiedadesCaptadas.some(p => p.estadoComercial === 'Disponible' || p.estadoComercial === 'En Negociación')
                ? 'bg-emerald-50 text-emerald-600 border-emerald-100/50'
                : 'bg-slate-50 text-slate-400 border-slate-100'
            }`}>
              Propietario: {cliente.propiedadesCaptadas && cliente.propiedadesCaptadas.some(p => p.estadoComercial === 'Disponible' || p.estadoComercial === 'En Negociación') ? 'Activo' : 'Cerrado'}
            </span>
          )}
        </div>
      </div>
