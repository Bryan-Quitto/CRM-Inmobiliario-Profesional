...
import { useAuditoriaGeneral } from '../hooks/useAuditoriaGeneral';
...
const SessionCard: React.FC<{ sesion: AuditoriaSessionResponse }> = ({ sesion }) => {
            ...
            {!isOpen && (
              <div className="flex items-center gap-4 mt-2 text-xs font-semibold text-slate-500">
                <span className="flex items-center gap-1.5">
                  ...
                  IA: {totalIA} {totalIA === 1 ? 'acción/msj' : 'acciones/msjs'}
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-slate-500"></span>
                  {sesion.canalPrincipal !== 'WhatsApp' && sesion.canalPrincipal !== 'Facebook' ? 'Agente' : 'Contacto'}: {totalContacto} {totalContacto === 1 ? 'mensaje' : 'mensajes'}
                </span>
              </div>
            )}
          </div>
...
export const AuditoriaGeneralView: React.FC = () => {
  const { sesiones, isLoading, error, dias, setDias, startDate, setStartDate, endDate, setEndDate, canal, setCanal } = useAuditoriaGeneral();
