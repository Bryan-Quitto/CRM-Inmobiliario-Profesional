...
export const PropiedadDetalleMobile = ({ id, onClose, logic }: Props) => {
  const {
    ...
    handleToggleArchive, handleWhatsAppShare
  } = logic;
    ...
    return (
      <div className="fixed inset-0 z-[200] flex lg:hidden bg-white items-center justify-center">
        <div className="flex flex-col items-center justify-center space-y-4 w-full px-4">
          <Loader2 className="h-12 w-12 text-indigo-600 animate-spin shrink-0" />
    ...
    <div className="fixed inset-0 z-[200] flex flex-col lg:hidden bg-white overflow-hidden">
      {syncing && (
        <div className="absolute top-20 right-4 z-[100] animate-in slide-in-from-top-4">
          <div className="bg-slate-900/90 text-white px-4 py-2 rounded-full shadow-2xl flex items-center gap-2 max-w-[90vw]">
            <Loader2 className="h-4 w-4 animate-spin text-indigo-400 shrink-0" />
            <span className="text-[10px] font-black uppercase tracking-[0.2em] truncate">Sync...</span>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-6 space-y-8 w-full">
        {activeTab === 'detalle' && (
          <>
            {/* Hero Info */}
            <div className="flex flex-col gap-4 w-full">
              <div className="flex flex-wrap items-center gap-2 w-full">
                ...
                <span className="text-[10px] font-black bg-slate-100 text-slate-500 border border-slate-200 px-2 py-0.5 rounded-md uppercase flex items-center gap-1 max-w-full">
                  ...
                  <span className="truncate flex-1 min-w-0">Captación: {propiedad.agenteNombre || 'Externa'}</span>
                </span>
              </div>
              
              <h1 className="text-2xl font-black text-slate-900 break-words leading-tight w-full">{propiedad.titulo}</h1>
              ...
              <div className="bg-indigo-50 p-4 rounded-2xl flex flex-col items-center w-full text-center">
                ...
                <p className="text-3xl font-black text-indigo-600 break-words w-full">{formatCurrency(propiedad.precio)}</p>
            ...
            <div className="flex flex-col gap-3 w-full">
              {['Casa', 'Departamento', 'Suite', 'Local Comercial', 'Oficina', 'Hotel'].includes(propiedad.tipoPropiedad) && (
                <div className="flex justify-between bg-slate-50 p-4 rounded-xl border border-slate-100 w-full gap-2">
                  <div className="flex items-center gap-2 text-slate-600 min-w-0"><Bed className="h-4 w-4 shrink-0" /> <span className="text-sm font-bold truncate">Habitaciones</span></div>
                  <span className="text-sm font-black text-slate-900 shrink-0">{propiedad.habitaciones || 0}</span>
                </div>
              )}
              {['Casa', 'Departamento', 'Suite', 'Local Comercial', 'Oficina', 'Hotel'].includes(propiedad.tipoPropiedad) && (
                <div className="flex justify-between bg-slate-50 p-4 rounded-xl border border-slate-100 w-full gap-2">
                  <div className="flex items-center gap-2 text-slate-600 min-w-0"><Bath className="h-4 w-4 shrink-0" /> <span className="text-sm font-bold truncate">Baños</span></div>
              ...
              {propiedad.estacionamientos && propiedad.estacionamientos > 0 ? (
                <div className="flex justify-between bg-slate-50 p-4 rounded-xl border border-slate-100 w-full gap-2">
                  <div className="flex items-center gap-2 text-slate-600 min-w-0"><CarFront className="h-4 w-4 shrink-0" /> <span className="text-sm font-bold truncate">Parqueaderos</span></div>
                  <span className="text-sm font-black text-slate-900 shrink-0">{propiedad.estacionamientos}</span>
                </div>
              ) : null}
              <div className="flex justify-between bg-indigo-50 p-4 rounded-xl border border-indigo-100 w-full gap-2">
                <div className="flex items-center gap-2 text-indigo-600 min-w-0"><Maximize className="h-4 w-4 shrink-0" /> <span className="text-sm font-bold truncate">Área Construcción</span></div>
                <span className="text-sm font-black text-indigo-900 shrink-0">{propiedad.areaConstruccion || 0} m²</span>
              </div>
              <div className="flex justify-between bg-slate-50 p-4 rounded-xl border border-slate-100 w-full gap-2">
                <div className="flex items-center gap-2 text-slate-600 min-w-0"><Ruler className="h-4 w-4 shrink-0" /> <span className="text-sm font-bold truncate">Área Total</span></div>
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              <p className="text-sm text-slate-600 leading-relaxed break-words whitespace-pre-wrap bg-slate-50 p-4 rounded-2xl w-full">
            ...
            <div className="flex flex-col gap-2 w-full overflow-hidden">
              ...
              {propiedad.media && propiedad.media.length > 0 ? (
                <div className="flex w-full overflow-x-auto gap-4 snap-x snap-mandatory pb-4">
                  {propiedad.media.map((img: MultimediaPropiedad) => (
                    <img 
                      key={img.id}
                      src={img.urlPublica}
                      alt="propiedad media"
                      className="w-[85vw] h-64 object-cover rounded-2xl snap-center shrink-0 border border-slate-200"
                    />
                  ))}
                </div>
              ) : (
                <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-6 flex flex-col items-center justify-center w-full">
