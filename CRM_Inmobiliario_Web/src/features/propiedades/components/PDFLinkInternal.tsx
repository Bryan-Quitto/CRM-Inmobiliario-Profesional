  ...
  if (status === 'missing') {
    return (
      <button 
        ...
        className="px-4 py-1.5 bg-white border-2 border-indigo-100 text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 hover:border-indigo-200 transition-all shadow-sm flex items-center gap-2 cursor-pointer active:scale-95"
      >
  ...
  return (
    <div className="flex items-center gap-1.5 animate-in fade-in zoom-in duration-500">
      <button 
        ...
        disabled={isDeleting}
        className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shadow-lg flex items-center gap-2 cursor-pointer active:scale-95 ${isDeleting ? 'bg-slate-100 text-slate-400 shadow-none' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-200'}`}
      ...
      <button
        onClick={handleGenerate}
        disabled={isDeleting}
        className="p-1.5 bg-white border border-slate-200 text-slate-400 rounded-full hover:text-indigo-600 hover:border-indigo-200 transition-all cursor-pointer disabled:opacity-30"
