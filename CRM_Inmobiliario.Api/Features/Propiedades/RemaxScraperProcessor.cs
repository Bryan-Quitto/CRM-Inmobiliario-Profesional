...
public static class RemaxScraperProcessor
{
    public static async Task<ImportarDeUrlRemaxFeature.ImportarRemaxResponse> ScrapeAsync(string url, HttpClient client)
    {
        var response = await client.GetAsync(url);
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception("No se pudo acceder a la página de Remax. HTTP " + response.StatusCode);
        }
        ...
        // Buscar encabezado formal de Ubicación
        ...
        return new ImportarDeUrlRemaxFeature.ImportarRemaxResponse(
            Titulo: titulo,
