...
public sealed class RegistrarNuevoContactoHandler : BaseWhatsAppToolHandler
{
    public RegistrarNuevoContactoHandler(CrmDbContext context, ILogger<RegistrarNuevoContactoHandler> logger) 
        : base(context, logger) { }
    ...
    public override async Task<string> ExecuteAsync(JsonDocument args, string phone, string triggerMessage, Contacto? contacto)
    {
        ...
        await LogAiActionAsync("Registro Contacto", args.RootElement.GetRawText(), phone, triggerMessage, newContacto.Id);
